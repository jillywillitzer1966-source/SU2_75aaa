...
su2double CNEMOGas::ComputerhoCvve() {
  ...
  Cvves = ComputeSpeciesCvVibEle(single_temperature ? T : Tve);
...
void CNEMOGas::ComputedPdU(const su2double *V, const vector<su2double>& val_eves, su2double *val_dPdU){
  ...
  unsigned long TVE_INDEX     = nSpecies+1;
  ...
  T = V[T_INDEX];
  Tve = single_temperature ? T : V[TVE_INDEX];
  ...
  if (ionization) {
    ...
    val_dPdU[0] = Ru*conc/rhoCvtr * (-ef + 0.5*sqvel) + Ru/MolarMass[0]*Tve
                - (single_temperature ? 0.0 : rho_el*Ru/MolarMass[0] * (-3.0/2.0*Ru/MolarMass[0]*Tve)/rhoCvve);
  }
  ...
  val_dPdU[nSpecies+nDim+1] = single_temperature ? 0.0 :
                               (-val_dPdU[nSpecies+nDim] + rho_el*Ru/MolarMass[0]*1.0/rhoCvve);
