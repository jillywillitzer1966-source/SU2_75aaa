...
CMutationTCLib::CMutationTCLib(const CConfig* config, unsigned short val_nDim): CNEMOGas(config, val_nDim){
  ...
  if (single_temperature) opt.setStateModel("ChemNonEq1T");
  else                    opt.setStateModel("ChemNonEqTTv");
...
void CMutationTCLib::SetTDStateRhosTTv(vector<su2double>& val_rhos, su2double val_temperature, su2double val_temperature_ve){
  ...
  temperatures[1] = single_temperature ? val_temperature : val_temperature_ve;
...
vector<su2double>& CMutationTCLib::ComputeSpeciesCvVibEle(su2double val_T){
   ...
   if (single_temperature) {
     for(iSpecies = 0; iSpecies < nSpecies; iSpecies++) Cvves[iSpecies] = Cv_ks[iSpecies];
   } else {
...
vector<su2double>& CMutationTCLib::ComputeSpeciesEve(su2double val_T, bool vibe_only){
  ...
  if (single_temperature) {
    for(iSpecies = 0; iSpecies < nSpecies; iSpecies++) eves[iSpecies] = es[iSpecies];
  } else {
...
vector<su2double>& CMutationTCLib::ComputeNetProductionRates(bool implicit, const su2double *V, const su2double* eve,
  ...
  /*--- For point-implicit chemistry in SU2, fill source Jacobian contributions. ---*/
  if (implicit && (val_jacobian != nullptr) && single_temperature) {
    vector<su2double> jac_species;
    vector<su2double> dwdT;
    jac_species.resize(nSpecies*nSpecies, 0.0);
    dwdT.resize(nSpecies, 0.0);

    mix->netProductionRatesJacobian1T(ws.data(), jac_species.data(), dwdT.data());
    ...
    /*--- Add d(omega)/dT * dT/dU chain-rule term (contains d(omega)/dE via dT/dU). ---*/
    if (dTdU != nullptr) {
      const unsigned short nVarLocal = nSpecies + nDim + 2;
      for (iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
        for (auto jVar = 0ul; jVar < nVarLocal; jVar++) {
    ...
    if (implicit && (val_jacobian != nullptr)) {
      vector<su2double> jac_species;
      jac_species.resize(nSpecies*nSpecies, 0.0);
      mix->jacobianRho(jac_species.data());
