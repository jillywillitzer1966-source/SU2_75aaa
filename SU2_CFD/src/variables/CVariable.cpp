...
CVariable::CVariable(unsigned long npoint, unsigned long ndim, unsigned long nvar,
  ...
  if (config->GetCompute_Metric()) {
    const unsigned short nHess = config->GetGoal_Oriented_Metric() ? nVar : config->GetnAdap_Sensor();
      const unsigned short nAuxAdap = config->GetGoal_Oriented_Metric() ? nDim + 3 : config->GetnAdap_Sensor();
      const unsigned short nMet = 3*(nDim-1);
    Gradient_Adapt.resize(nPoint,nHess,nDim,0.0);
      AuxVar_Adapt.resize(nPoint,nAuxAdap) = su2double(0.0);
      Gradient_AuxVar_Adapt.resize(nPoint,nAuxAdap,nDim,0.0);
    Hessian.resize(nPoint,nHess,3*(nDim-1),0.0);
      Metric.resize(nPoint,nMet) = 0.0;
  }
}
