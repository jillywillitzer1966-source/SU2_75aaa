...
CNEMOEulerVariable::CNEMOEulerVariable(su2double val_pressure,
  ...
  if (config->GetKind_NEMOTemperatureModel() == NEMO_TEMPERATURE_MODEL::SINGLE_TEMPERATURE) {
    Tve_Freestream = val_temperature;
  }
...
bool CNEMOEulerVariable::Cons2PrimVar(su2double *U, su2double *V,
  ...
  if (single_temperature) {
    su2double Told = V[T_INDEX];
    if (Told < Tmin || Told > Tmax || !(Told == Told)) Told = 300.0;
    fluidmodel->SetTDStateRhosTTv(rhos, Told, Told);
    const auto& eves_old = fluidmodel->ComputeSpeciesEve(Told);
    rhoEve = 0.0;
    for (iSpecies = 0; iSpecies < nSpecies; iSpecies++)
      rhoEve += rhos[iSpecies]*eves_old[iSpecies];
  }
  ...
  V[T_INDEX]   = T[0];
  V[TVE_INDEX] = T[1];
  if (single_temperature) V[TVE_INDEX] = V[T_INDEX];
  ...
  if (single_temperature) {
    su2double rhoEve_eq = 0.0;
    for (iSpecies = 0; iSpecies < nSpecies; iSpecies++)
      rhoEve_eq += rhos[iSpecies]*val_eves[iSpecies];
    U[nSpecies+nDim+1] = rhoEve_eq;
  }
