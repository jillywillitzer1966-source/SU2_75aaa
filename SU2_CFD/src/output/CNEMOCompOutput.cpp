...
void CNEMOCompOutput::LoadVolumeData(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned long iPoint){
  ...
  if (nDim == 3){
    ...
    if (config->GetKind_NEMOTemperatureModel() == NEMO_TEMPERATURE_MODEL::SINGLE_TEMPERATURE) {
      SetVolumeOutputValue("ENERGY_VE",  iPoint, Node_Flow->GetSolution(iPoint, nSpecies+2));
    } else {
      SetVolumeOutputValue("ENERGY_VE",  iPoint, Node_Flow->GetSolution(iPoint, nSpecies+3));
    }
  }
  ...
  if (config->GetKind_NEMOTemperatureModel() == NEMO_TEMPERATURE_MODEL::SINGLE_TEMPERATURE) {
    SetVolumeOutputValue("TEMPERATURE_VE", iPoint, Node_Flow->GetTemperature(iPoint));
  } else {
  ...
  if (nDim == 3){
    ...
    if (config->GetKind_NEMOTemperatureModel() == NEMO_TEMPERATURE_MODEL::SINGLE_TEMPERATURE) {
      SetVolumeOutputValue("RES_ENERGY_VE", iPoint, solver[FLOW_SOL]->LinSysRes(iPoint, nSpecies+2));
    } else {
      SetVolumeOutputValue("RES_ENERGY_VE", iPoint, solver[FLOW_SOL]->LinSysRes(iPoint, nSpecies+3));
    }
  }
...
void CNEMOCompOutput::LoadHistoryData(CConfig *config, CGeometry *geometry, CSolver **solver)  {
  ...
  if (nDim == 2){
    ...
    if (config->GetKind_NEMOTemperatureModel() == NEMO_TEMPERATURE_MODEL::SINGLE_TEMPERATURE) {
      SetHistoryOutputValue("RMS_ENERGY_VE", log10(NEMO_solver->GetRes_RMS(nSpecies+2)));
    } else {
