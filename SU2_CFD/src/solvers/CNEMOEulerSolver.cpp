...
CNEMOEulerSolver::CNEMOEulerSolver(CGeometry *geometry, CConfig *config,
  ...
  if (UseGlobalImplicitAssembly(config)) {
  ...
  if (config->GetKind_NEMOTemperatureModel() == NEMO_TEMPERATURE_MODEL::SINGLE_TEMPERATURE) {
    Temperature_ve_Inf = Temperature_Inf;
    Energy_ve_Inf      = Energy_Inf;
  }
...
bool CNEMOEulerSolver::IsTruePointImplicitMode(const CConfig* config) const {
...
bool CNEMOEulerSolver::UseGlobalImplicitAssembly(const CConfig* config) const {
  if (config->GetKind_TimeIntScheme_Flow() == EULER_IMPLICIT) return true;
  if (config->GetKind_TimeIntScheme_Flow() == POINT_IMPLICIT) return !IsTruePointImplicitMode(config);
  return false;
}

void CNEMOEulerSolver::AssembleLocalChemJacobian(unsigned long iPoint, const CGeometry* geometry,
                                                 const CConfig* config, su2double** local_jac) const {

  if (config->GetFrozen() || config->GetMonoatomic()) return;
  ...
  const su2double* V = nodes->GetPrimitive(iPoint);
  ...
  FluidModel->ComputeNetProductionRates(true, V, nodes->GetEve(iPoint), nodes->GetCvve(iPoint),
                                        nodes->GetdTdU(iPoint), nodes->GetdTvedU(iPoint), local_jac);
...
void CNEMOEulerSolver::CommonPreprocessing(CGeometry *geometry, CSolver **solver_container, CConfig *config, unsigned short iMesh,
  ...
  const bool implicit = UseGlobalImplicitAssembly(config);
...
void CNEMOEulerSolver::RecomputeConservativeVector(su2double *U, const su2double *V) const {
  ...
  const bool single_temperature = FluidModel->GetSingleTemperature();
  const su2double Tve = single_temperature ? V[T_INDEX] : V[TVE_INDEX];
  FluidModel->SetTDStateRhosTTv(rhos, V[T_INDEX], Tve);
  ...
  if (single_temperature) {
    const auto& eves = FluidModel->ComputeSpeciesEve(V[T_INDEX]);
    su2double rhoEve = 0.0;
    for (auto iSpecies = 0ul; iSpecies < nSpecies; iSpecies++)
      rhoEve += rhos[iSpecies]*eves[iSpecies];
...
void CNEMOEulerSolver::PointImplicit_Iteration(CGeometry *geometry, CSolver **solver_container, CConfig *config) {

  if (!IsTruePointImplicitMode(config)) {
    CFVMFlowSolverBase<CNEMOEulerVariable, ENUM_REGIME::COMPRESSIBLE>::PointImplicit_Iteration(geometry, solver_container, config);
    return;
  }

  /*--- Build the linear right-hand-side from explicit residuals, without global Jacobian assembly. ---*/
  ...
  for (unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++) {
    ...
    su2double A[MAXNVAR][MAXNVAR] = {{0.0}};
    su2double Jchem[MAXNVAR][MAXNVAR] = {{0.0}};
    su2double rhs[MAXNVAR] = {0.0};
    su2double* A_rows[MAXNVAR] = {nullptr};
    su2double* Jchem_rows[MAXNVAR] = {nullptr};

    for (unsigned short iVar = 0; iVar < nVar; iVar++) {
      A_rows[iVar] = A[iVar];
      Jchem_rows[iVar] = Jchem[iVar];
    }
    ...
    for (unsigned short iVar = 0; iVar < nVar; iVar++) {
      A[iVar][iVar] = delta;
      rhs[iVar] = LinSysRes(iPoint, iVar);
    }

    AssembleLocalChemJacobian(iPoint, geometry, config, Jchem_rows);
    ...
    for (unsigned short iVar = 0; iVar < nVar; iVar++) {
      LinSysSol(iPoint, iVar) = rhs[iVar];
    }
  }
  END_SU2_OMP_FOR
  ...
  su2double localRes = 0.0, globalRes = 0.0;
  for (unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++) {
    for (unsigned short iVar = 0; iVar < nVar; iVar++) {
      localRes = max(localRes, fabs(LinSysSol(iPoint, iVar)));
    }
  }

  BEGIN_SU2_OMP_SAFE_GLOBAL_ACCESS {
    su2double myRes = localRes;
    SU2_MPI::Allreduce(&myRes, &globalRes, 1, MPI_DOUBLE, MPI_MAX, SU2_MPI::GetComm());
    SetIterLinSolver(1);
    SetResLinSolver(max(globalRes, EPS));
  }
  END_SU2_OMP_SAFE_GLOBAL_ACCESS
...
void CNEMOEulerSolver::CompleteImplicitIteration(CGeometry *geometry, CSolver**, CConfig *config) {
  ...
  if (!config->GetContinuous_Adjoint()) {
    ...
    /*--- Positivity clipping and strict species-mass renormalization for true point-implicit mode. ---*/
    const su2double rho_floor = 1e-20;
    SU2_OMP_FOR_STAT(omp_chunk_size)
    for (unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++) {
      su2double rho_target = 0.0;
      su2double rho_sum = 0.0;

      for (unsigned short iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
        rho_target += nodes->GetSolution(iPoint, iSpecies);
      }
      rho_target = max(rho_target, nSpecies*rho_floor);

      for (unsigned short iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
        const su2double rho_clip = max(nodes->GetSolution(iPoint, iSpecies), rho_floor);
        nodes->SetSolution(iPoint, iSpecies, rho_clip);
        rho_sum += rho_clip;
      }

      if (rho_sum > EPS) {
        const su2double scale = rho_target / rho_sum;
        for (unsigned short iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
          nodes->SetSolution(iPoint, iSpecies, nodes->GetSolution(iPoint, iSpecies) * scale);
        }
      } else {
...
void CNEMOEulerSolver::BC_Inlet(CGeometry *geometry, CSolver **solver_container,
  ...
  bool implicit             = UseGlobalImplicitAssembly(config);
  ...
  for (auto iVertex = 0ul; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->nodes->GetDomain(iPoint)) {
      ...
      if (Mach_Exit >= 1.0) {
        ...
        if (FluidModel->GetSingleTemperature()) {
          const auto& eves = FluidModel->ComputeSpeciesEve(Temperature);
          su2double rhoEve = 0.0;
          for (auto iSpecies = 0ul; iSpecies < nSpecies; iSpecies++)
            rhoEve += rhos[iSpecies] * eves[iSpecies];
          U_outlet[nVar-1] = rhoEve;
        } else {
