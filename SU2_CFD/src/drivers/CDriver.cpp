...
void CDriver::InitializeNumerics(CConfig *config, CGeometry **geometry, CSolver ***solver, CNumerics ****&numerics) const {
  ...
  bool euler, detonation_euler, ns, NEMO_euler, NEMO_ns, turbulent, adj_euler, adj_ns, adj_turb, fem_euler, fem_ns;
  bool fem, heat, transition, template_solver;

  euler = detonation_euler = ns = NEMO_euler = NEMO_ns = turbulent = adj_euler = adj_ns = adj_turb = fem_euler = fem_ns = false;
  ...
  switch (config->GetKind_Solver()) {
    ...
    case MAIN_SOLVER::DETONATION_EULER:
      detonation_euler = compressible = true; break;
  ...
  if (detonation_euler) nVar_Detonation = solver[MESH_0][FLOW_SOL]->GetnVar();
  ...
  if (detonation_euler) nPrimVar_Detonation = solver[MESH_0][FLOW_SOL]->GetnPrimVar();
  ...
  if (detonation_euler) {
    if (config->GetKind_ConvNumScheme_Flow() != SPACE_UPWIND ||
        (config->GetKind_Upwind_Flow() != UPWIND::DETONATION_LLF &&
         config->GetKind_Upwind_Flow() != UPWIND::DETONATION_HLLC)) {
      SU2_MPI::Error("DETONATION_EULER Phase-1 requires SPACE_UPWIND + DETONATION_LLF or DETONATION_HLLC numerics.", CURRENT_FUNCTION);
    }

    for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
      if (config->GetKind_Upwind_Flow() == UPWIND::DETONATION_HLLC) {
        numerics[iMGlevel][FLOW_SOL][CONV_TERM] = new CUpwHLLC_Detonation(nDim, nVar_Detonation,
                                                                          nPrimVar_Detonation, config);
        numerics[iMGlevel][FLOW_SOL][CONV_BOUND_TERM] = new CUpwHLLC_Detonation(nDim, nVar_Detonation,
                                                                                nPrimVar_Detonation, config);
      } else {
        numerics[iMGlevel][FLOW_SOL][CONV_TERM] = new CUpwLLF_Detonation(nDim, nVar_Detonation,
                                                                         nPrimVar_Detonation, config);
        numerics[iMGlevel][FLOW_SOL][CONV_BOUND_TERM] = new CUpwLLF_Detonation(nDim, nVar_Detonation,
                                                                               nPrimVar_Detonation, config);
      }
      numerics[iMGlevel][FLOW_SOL][VISC_TERM] = new CSourceNothing(nDim, nVar_Detonation, config);
      numerics[iMGlevel][FLOW_SOL][VISC_BOUND_TERM] = new CSourceNothing(nDim, nVar_Detonation, config);
      numerics[iMGlevel][FLOW_SOL][SOURCE_FIRST_TERM] = new CSourceNothing(nDim, nVar_Detonation, config);
      numerics[iMGlevel][FLOW_SOL][SOURCE_SECOND_TERM] = new CSourceNothing(nDim, nVar_Detonation, config);
    }
  }
...
void CHBDriver::SetHarmonicBalance(unsigned short iInst) {
  ...
  bool implicit = (config_container[ZONE_0]->GetKind_TimeIntScheme_Flow() == EULER_IMPLICIT) ||
                  (config_container[ZONE_0]->GetKind_TimeIntScheme_Flow() == POINT_IMPLICIT);
