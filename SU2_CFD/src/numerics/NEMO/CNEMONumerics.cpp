...
namespace {
inline bool IsTruePointImplicitMode(const CConfig* config) {
...
inline bool UseGlobalImplicitAssembly(const CConfig* config) {
...
CNEMONumerics::CNEMONumerics(unsigned short val_nDim, unsigned short val_nVar,
    ...
    /*--- Read from CConfig ---*/
    implicit   = UseGlobalImplicitAssembly(config);
    single_temperature = (config->GetKind_NEMOTemperatureModel() == NEMO_TEMPERATURE_MODEL::SINGLE_TEMPERATURE);
...
void CNEMONumerics::GetInviscidProjFlux(const su2double *val_U,
  ...
  const su2double rhoEve = single_temperature ? 0.0 : val_U[nSpecies+nDim+1];
...
void CNEMONumerics::GetInviscidProjJac(const su2double *val_U,    const su2double *val_V,
  ...
  if (single_temperature) {
    const auto iVe = nSpecies + nDim + 1;
    for (auto jVar = 0ul; jVar < nVar; jVar++) {
      val_Proj_Jac_Tensor[iVe][jVar] = 0.0;
      val_Proj_Jac_Tensor[jVar][iVe] = 0.0;
    }
  }
}
