...
CNumerics::ResidualType<> CSource_NEMO::ComputeAxisymmetric(const CConfig *config) {
  ...
  if (viscous) {
    ...
    const su2double total_conductivity_i =
      (single_temperature ? (ktr + heat_capacity_cp_i*Eddy_Viscosity_i/Prandtl_Turb)
                : (ktr + kve + heat_capacity_cp_i*Eddy_Viscosity_i/Prandtl_Turb));
    const su2double u                    = V_i[VEL_INDEX];
    const su2double v                    = V_i[VEL_INDEX+1];
    const su2double qy_t                 = -total_conductivity_i*GV[T_INDEX][1];
    const su2double qy_ve                = single_temperature ? 0.0 : -kve*GV[TVE_INDEX][1];
