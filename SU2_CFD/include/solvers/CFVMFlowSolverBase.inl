...
void CFVMFlowSolverBase<V, R>::CommunicateInitialState(CGeometry* geometry, const CConfig* config) {
  ...
  euler_implicit = (config->GetKind_TimeIntScheme_Flow() == EULER_IMPLICIT) ||
                   (config->GetKind_TimeIntScheme_Flow() == POINT_IMPLICIT);
...
void CFVMFlowSolverBase<V, R>::Viscous_Residual_impl(unsigned long iEdge, CGeometry *geometry, CSolver **solver_container,
  ...
  const bool implicit  = (config->GetKind_TimeIntScheme() == EULER_IMPLICIT) ||
                         (config->GetKind_TimeIntScheme() == POINT_IMPLICIT);
...
template <class V, ENUM_REGIME R>
void CFVMFlowSolverBase<V, R>::PointImplicit_Iteration(CGeometry *geometry, CSolver **solver_container, CConfig *config) {
  PrepareImplicitIteration(geometry, nullptr, config);

  /*--- Ensure halo entries do not carry stale values before point solves. ---*/
  SU2_OMP_FOR_(schedule(static,OMP_MIN_SIZE) SU2_NOWAIT)
  for (unsigned long iPoint = nPointDomain; iPoint < nPoint; iPoint++) {
    LinSysRes.SetBlock_Zero(iPoint);
    LinSysSol.SetBlock_Zero(iPoint);
  ...
  /*--- True point-implicit solve: iterative block-Jacobi on A*dx = b. ---*/
  CSysVector<su2double> JacobiResidual;
  CSysVector<su2double> JacobiCorrection;
  JacobiResidual.Initialize(nPoint, nPointDomain, nVar, 0.0);
  JacobiCorrection.Initialize(nPoint, nPointDomain, nVar, 0.0);

  const unsigned long max_inner_iter = max(1ul, config->GetLinear_Solver_Iter());
  const su2double linear_tol = max(config->GetLinear_Solver_Error(), EPS);
  const su2double omega = 1.0;

  Jacobian.BuildJacobiPreconditioner();

  su2double globalRes = 0.0;
  unsigned long iter_used = 0;
  vector<su2double> threadRes(omp_get_max_threads(), 0.0);

  for (unsigned long iter = 0; iter < max_inner_iter; iter++) {

    /*--- Residual of the linearized system: r = A*dx - b. ---*/
    Jacobian.ComputeResidual(LinSysSol, LinSysRes, JacobiResidual);

    fill(threadRes.begin(), threadRes.end(), 0.0);

    SU2_OMP_FOR_STAT(omp_chunk_size)
    for (unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++) {
      su2double pointRes = 0.0;
      for (unsigned short iVar = 0; iVar < nVar; iVar++) {
        pointRes = max(pointRes, fabs(JacobiResidual(iPoint, iVar)));
      }
      threadRes[omp_get_thread_num()] = max(threadRes[omp_get_thread_num()], pointRes);
    }
    END_SU2_OMP_FOR

    su2double localRes = 0.0;
    for (auto res : threadRes) localRes = max(localRes, res);
    ...
    iter_used = iter + 1;
    if (globalRes <= linear_tol) break;

    /*--- Block-Jacobi correction: delta = D^{-1} r, update dx <- dx - delta. ---*/
    Jacobian.ComputeJacobiPreconditioner(JacobiResidual, JacobiCorrection, geometry, config);

    SU2_OMP_FOR_STAT(omp_chunk_size)
    for (unsigned long iPoint = 0; iPoint < nPoint; iPoint++) {
      for (unsigned short iVar = 0; iVar < nVar; iVar++) {
        LinSysSol(iPoint, iVar) -= omega * JacobiCorrection(iPoint, iVar);
      }
    }
    END_SU2_OMP_FOR
  }
  ...
  BEGIN_SU2_OMP_SAFE_GLOBAL_ACCESS {
    SetIterLinSolver(static_cast<unsigned short>(min(iter_used, 65535ul)));
