...
protected:
  ...
  static constexpr size_t MAXNDIM = 3; /*!< \brief Max number of space dimensions for static work arrays. */
...
public:
  ...
  /*--- Mesh adaptation support methods. ---*/
 ...
 /*!
  ...
  /*!
   * \brief A virtual member for point-implicit local block iterations.
   * \param[in] geometry - Geometrical definition of the problem.
   * \param[in] solver_container - Container vector with all the solutions.
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void PointImplicit_Iteration(CGeometry *geometry,
                                              CSolver **solver_container,
                                              CConfig *config) {
    ImplicitEuler_Iteration(geometry, solver_container, config);
  }
  ...
  */
  ...
  /*!
   * \brief Correct the gradient at wall boundaries.
   */
  ...
  /*!
   * \brief Correct NaNs in the metric field at boundaries.
   */
  void CorrectNaNsBoundMetric(CGeometry *geometry, const CConfig *config);

  /*!
   * \brief Compute the goal-oriented or feature-based metric.
   */
  void ComputeMetric(CSolver **solver, CGeometry *geometry, const CConfig *config);

  /*!
   * \brief Extract objective-function terms for the goal-oriented metric.
   */
  ...
  /*!
   * \brief Perform Lp normalization of the metric tensor field.
   */
  template<class MetType, class Metric>
  void NormalizeMetric(const CGeometry *geometry, const CConfig *config, unsigned short iSensor) {
    ...
    const MetType outComplex = MetType(config->GetAdap_Complexity());
    ...
    for (auto iPoint = 0ul; iPoint < nPointDomain; ++iPoint) {
      ...
      const MetType factor = pow(outComplex/globalScale, 2./nDim) *
                             pow(abs(EigVal[0]*EigVal[1]*EigVal[2]), -1./(2.*p+nDim));
