...
protected:
  ...
  /*!\
   * \brief Return true if the run should use the true local point-implicit mode.
   */
  bool IsTruePointImplicitMode(const CConfig* config) const;

  /*!\
   * \brief Return true when global Jacobian assembly should be used.
   */
  bool UseGlobalImplicitAssembly(const CConfig* config) const;

  /*!\
   * \brief Assemble local chemistry Jacobian for one point in true point-implicit mode.
   */
  void AssembleLocalChemJacobian(unsigned long iPoint, const CGeometry* geometry,
...
public:
  ...
  /*!\
   * \brief Update the solution using a point-implicit scheme.
   * \param[in] geometry - Geometrical definition of the problem.
   * \param[in] solver_container - Container vector with all the solutions.
   * \param[in] config - Definition of the particular problem.
   */
  void PointImplicit_Iteration(CGeometry *geometry, CSolver **solver_container, CConfig *config) override;
