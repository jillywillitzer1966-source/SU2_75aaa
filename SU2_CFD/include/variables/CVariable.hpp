...
protected:
  ...
  MatrixType AuxVar_Adapt;                /*!< \brief Variables for which gradients are needed for mesh adaptation. */
...
public:
  ...
  /*!\
   * \brief Set the adaptation sensor gradient at a point.
   */
  ...
  /*!\
   * \brief Get reference to adaptation sensor gradients.
   */
  inline CVectorOfMatrix& GetGradient_Adapt(void) { return Gradient_Adapt; }

  /*!\
   * \brief Get adaptation sensor gradient matrix at one point.
   */
  inline CMatrixView<su2double> GetGradient_Adapt(unsigned long iPoint) { return Gradient_Adapt[iPoint]; }

  /*!\
   * \brief Get one component of adaptation sensor gradient.
   */
  inline su2double GetGradient_Adapt(unsigned long iPoint, unsigned long iVar, unsigned long iDim) const {
    return Gradient_Adapt(iPoint,iVar,iDim);
  }

  /*!\
   * \brief Set one adaptation auxiliary variable value.
   */
  ...
  /*!\
   * \brief Get reference to adaptation auxiliary gradients.
   */
  inline CVectorOfMatrix& GetGradient_AuxVar_Adapt(void) { return Gradient_AuxVar_Adapt; }

  /*!\
   * \brief Get adaptation auxiliary gradient matrix at one point.
   */
  inline CMatrixView<su2double> GetGradient_AuxVar_Adapt(unsigned long iPoint) { return Gradient_AuxVar_Adapt[iPoint]; }

  /*!\
   * \brief Get one component of adaptation auxiliary gradient.
   */
  inline su2double GetGradient_AuxVar_Adapt(unsigned long iPoint, unsigned long iVar, unsigned long iDim) const {
    return Gradient_AuxVar_Adapt(iPoint,iVar,iDim);
  }

  /*!\
   * \brief Set one Hessian component.
   */
  inline void SetHessian(unsigned long iPoint, unsigned long iVar, unsigned long iHess, su2double hess) {
    Hessian(iPoint,iVar,iHess) = hess;
  }

  /*!\
   * \brief Set packed upper-triangle Hessian entries from a full matrix.
   */
  template <class Met>
  ...
  /*!\
   * \brief Get reference to Hessian tensor storage.
   */
  inline CVectorOfMatrix& GetHessian(void) { return Hessian; }

  /*!\
   * \brief Get one Hessian component.
   */
  inline su2double GetHessian(unsigned long iPoint, unsigned long iVar, unsigned long iHess) const {
    return Hessian(iPoint,iVar,iHess);
  }

  /*!\
   * \brief Get full Hessian matrix from packed storage.
   */
  template <class Met>
