...
private:
  ...
  bool custom_chemistry_tables = false; /*!< \brief Enable custom SU2_NONEQ chemistry tables. */
  ...
  *dPdU, *dTdU, *dTvedU;
  ...
  vector<unsigned short> KeqTableIndex;        /*!< \brief Keq table id associated to each reaction. */
  vector<array<su2double,5>> CustomKeqCoeff;   /*!< \brief Fitted Keq coefficients for each reaction. */
...
public:
  ...
  /*!\
   * \brief Load custom SU2_NONEQ chemistry tables from CSV files.
   */
  void LoadCustomChemistryTables(const CConfig* config, bool viscous);
