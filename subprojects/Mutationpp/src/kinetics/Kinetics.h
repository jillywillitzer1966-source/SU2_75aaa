...
public:
    ...
    /**
     * Computes the net species production rates together with the analytic
     * single-temperature chemistry Jacobian pieces
     * \f[
     * \left.\frac{\partial \dot{\omega}_i}{\partial \rho_j}\right|_T
     * \quad \text{and} \quad
     * \left.\frac{\partial \dot{\omega}_i}{\partial T}\right|_{\rho}
     * \f]
     * using the current thermochemical state. This interface is only valid for
     * the 1T state model.
     *
     * @param p_wdot  on return, species production rates in kg/m^3-s
     * @param p_jac   on return, row-major species Jacobian wrt densities
     * @param p_dwdt  on return, temperature derivative of the species source
     */
    void netProductionRatesJacobian1T(
