...
void Kinetics::netProductionRatesJacobian1T(
    double* const p_wdot, double* const p_jac, double* const p_dwdt)
{
    const int ns = m_thermo.nSpecies();
    const int nr = nReactions();

    if (m_thermo.nEnergyEqns() != 1) {
        throw NotImplementedError("Kinetics::netProductionRatesJacobian1T()")
            << "Only the 1T state model is supported by this interface.";
    }

    if (nr == 0) {
        if (p_wdot != NULL)
            std::fill(p_wdot, p_wdot + ns, 0.0);
        if (p_jac != NULL)
            std::fill(p_jac, p_jac + ns*ns, 0.0);
        if (p_dwdt != NULL)
            std::fill(p_dwdt, p_dwdt + ns, 0.0);
        return;
    }

    mp_rates->update(m_thermo);

    Map<ArrayXd>(mp_ropf, nr) = Map<const ArrayXd>(mp_rates->lnkf(), nr).exp();
    Map<ArrayXd>(mp_ropb, nr) = Map<const ArrayXd>(mp_rates->lnkb(), nr).exp();

    for (int i = 0; i < mp_rates->irrReactions().size(); ++i)
        mp_ropb[mp_rates->irrReactions()[i]] = 0.0;

    std::vector<double> conc(ns, 0.0);
    std::vector<double> h_over_rt(ns, 0.0);
    const double T = m_thermo.T();
    const double conc_scale = m_thermo.numberDensity() / NA;

    for (int i = 0; i < ns; ++i)
        conc[i] = conc_scale * m_thermo.X()[i];

    m_reactants.multReactions(conc.data(), mp_ropf);
    m_rev_prods.multReactions(conc.data(), mp_ropb);
    m_thirdbodies.multiplyThirdbodies(conc.data(), mp_ropf);
    m_thirdbodies.multiplyThirdbodies(conc.data(), mp_ropb);

    if (p_jac != NULL)
        m_jacobian.computeJacobian(mp_ropf, mp_ropb, conc.data(), p_jac);

    if (p_wdot != NULL || p_dwdt != NULL)
        m_thermo.speciesHOverRT(T, h_over_rt.data());

    if (p_wdot != NULL) {
        Map<ArrayXd>(mp_rop, nr) =
            Map<const ArrayXd>(mp_ropf, nr) - Map<const ArrayXd>(mp_ropb, nr);
    ...
    if (p_dwdt != NULL) {
        ...
        for (int i_rxn = 0; i_rxn < nr; ++i_rxn) {
            const Reaction& reaction = m_reactions[i_rxn];
            const Arrhenius* const p_rate =
                dynamic_cast<const Arrhenius*>(reaction.rateLaw());

            if (p_rate == NULL) {
                throw NotImplementedError("Kinetics::netProductionRatesJacobian1T()")
                    << "Only Arrhenius rate laws are supported.";
            }

            const double dlnkf_dT =
                (p_rate->n() + p_rate->T()/T) / T;

            double dlnkb_dT = 0.0;
            if (reaction.isReversible()) {
                double delta_h_over_rt = 0.0;

                const std::vector<int>& reactants = reaction.reactants();
                const std::vector<int>& products  = reaction.products();

                for (int i = 0; i < reactants.size(); ++i)
                    delta_h_over_rt -= h_over_rt[reactants[i]];
                for (int i = 0; i < products.size(); ++i)
                    delta_h_over_rt += h_over_rt[products[i]];

                const double delta_nu =
                    static_cast<double>(reaction.nProducts() - reaction.nReactants());
            ...
            for (int i = 0; i < reactants.size(); ++i) {
                const int i_sp = reactants[i];
                p_dwdt[i_sp] -= m_thermo.speciesMw(i_sp) * drop_dT;
            }

            for (int i = 0; i < products.size(); ++i) {
                const int i_sp = products[i];
                p_dwdt[i_sp] += m_thermo.speciesMw(i_sp) * drop_dT;
            }
        }
    }
